...
def plot_roc_curve_binary(y_true: list, y_pred: list) -> None:
    ...
    plt.savefig("../output/dataset-{}_model-{}_imagesize-{}_ROC-binary.png".format(config.dataset, config.model, config.image_size))
    plt.show()
...
def plot_roc_curve_multiclass(y_true: list, y_pred: list, label_encoder) -> None:
    ...
    plt.title('Receiver Operating Characteristic')
    plt.legend(loc='lower right')
    plt.savefig("../output/dataset-{}_model-{}_imagesize-{}_ROC-multi.png".format(config.dataset, config.model, config.image_size))
    plt.show()
...
def plot_confusion_matrix(cm: np.ndarray, fmt: str, label_encoder, is_normalised: bool) -> None:
    ...
    if is_normalised:
        plt.savefig("../output/dataset-{}_model-{}_imagesize-{}_CM-norm.png".format(config.dataset, config.model, config.image_size))
    elif not is_normalised:
        plt.savefig("../output/dataset-{}_model-{}_imagesize-{}_CM.png".format(config.dataset, config.model, config.image_size))
    plt.show()
...
def plot_training_results(hist_input, plot_name: str, is_frozen_layers) -> None:
    ...
    plt.savefig("../output/dataset-{}_model-{}_imagesize-{}_{}.png".format(config.dataset, config.model, config.image_size, plot_name))
    plt.show()
